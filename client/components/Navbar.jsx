...
const Navbar = ({ isDarkMode, setIsDarkMode }) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        console.log("BoardList", list);const currentBoard = list.find((board) => board._id === currBoardID);
        setCurrBoardName(currentBoard.name);
        setInputValue(currentBoard.name);
      })
      .catch((error) => console.error(error));
  }, [user._id, currBoardID]);
  ...
  const boardOptions =
    ...
    boardList.map((board) => (
      <DropdownItem
        boardID={board._id}
        onClick={() => setCurrBoardID(board._id)}
      >
        {board.name}
      </DropdownItem>
    ));
  ...
  return (
    <nav className={`navbar`}>
      <ul className="navbar-nav">
        <NavItem>
          <DropdownMenu>{boardOptions}</DropdownMenu>
        </NavItem>
